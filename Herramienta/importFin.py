...
def installDrupal(nombreSitio):
    ...
    info = subprocess.Popen(['mv', 'drupal-8.8.4/',cad]).wait()#,stdout=subprocess.PIPE,stderr=subprocess.STDOUT).wait()
    if info != 0:
    ...
    password=input("Ingrese la contrasena de acceso.\n")
    #user='drupaluser'
    #ip='10.0.0.2'
    #port='5432'
    #bd = 'drupal81'
    ...
    info = subprocess.Popen(['drush','-y', 'config-set', 'system.perfermance','css.preprocess','0','-r',cad]).wait()#,stdout=subprocess.PIPE,stderr=subprocess.STDOUT).wait()
    if info != 0:
    ...
    print("Configuracion de sitio")
    info = subprocess.Popen(['drush','-y', 'config-set', 'system.perfermance','js.preprocess','0','-r',cad]).wait()#,stdout=subprocess.PIPE,stderr=subprocess.STDOUT).wait()
    if info != 0:
        print("js.preprocess")
        print("Abortando operacion")
        sys.exit()
